  ...
  try {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      final roleUrl = Uri.parse(
          'https://eliuwjh1sfjv.share.zrok.io/api/getrole?firstname=${_firstNameCtrl.text.trim()}&lastname=${_lastNameCtrl.text.trim()}');
      ...
      if (responseData.containsKey('role')) {
        ...
        if (userRole != 'customer' && userRole != 'admin') {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text('Unexpected role assigned. Please contact support.')),
          );
          return;
        }
        ...
        if (idResponse.statusCode == 200) {
          final userDetails = jsonDecode(idResponse.body);
          if (userDetails.containsKey('id')) {
            await box.put('customerId', userDetails['id']); // Save customer ID
          } else {
            // Optionally handle missing id key
            ScaffoldMessenger.of(context).showSnackBar(
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Failed to get user details: ${idResponse.statusCode}')),
          );
        }
